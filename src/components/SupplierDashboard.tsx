...
import { Package, DollarSign, Clock, TrendingUp, Plus, Edit3, Eye, CheckCircle } from 'lucide-react';
import Navbar from './common/Navbar';
import LoadingSpinner from './common/LoadingSpinner';
import { dbQueries } from '../lib/supabase';
import { formatCurrency } from '../utils/helpers';
import type { Product } from '../lib/tableNames';
...
const SupplierDashboard = ({ user, onLogout }: SupplierDashboardProps) => {
  ...
  useEffect(() => {
    const fetchSupplierData = async () => {
      ...
      try {
        ...
        const { data: productsData, error: productsError } = await dbQueries.getProductsBySupplier(user.id);
        if (productsError) throw productsError;

        // Fetch supplier's orders
        const { data: ordersData, error: ordersError } = await dbQueries.getSupplierOrders(user.id);
  ...
  const updateOrderStatus = async (orderId: number, newStatus: string) => {
    try {
      const { error } = await dbQueries.updateOrderStatus(orderId, newStatus);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gray-50">
        <Navbar user={user} userType="supplier" onLogout={onLogout} />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Manage your inventory and fulfill orders from food vendors
          </p>
        </div>
        ...
        {activeTab === 'orders' && (
          <div className="bg-white rounded-lg shadow-md">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">Incoming Orders</h3>
              <p className="text-sm text-gray-600">Manage and fulfill vendor orders</p>
            </div>
                    ...
                    <div className="flex items-center justify-between mb-4">
                      <div>
                        ...
                        <p className="text-sm text-gray-600">
                          From: {order.vendor?.business_name || order.vendor?.name} • {order.vendor?.area}
                        </p>
          ...
          <div className="bg-white rounded-lg shadow-md">
            <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
              <div>
                ...
                <p className="text-sm text-gray-600">Manage your product catalog and stock levels</p>
              </div>
        ...
        {activeTab === 'analytics' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <h3 className="text-lg font-medium text-gray-900 mb-6">Performance Analytics</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="p-4 border border-gray-200 rounded-lg">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Order Status Distribution</h4>
                <div className="space-y-2">
                  {['pending', 'confirmed', 'preparing', 'dispatched', 'delivered'].map(status => {
              ...
              <div className="p-4 border border-gray-200 rounded-lg">
                <h4 className="text-sm font-medium text-gray-700 mb-2">Revenue Summary</h4>
